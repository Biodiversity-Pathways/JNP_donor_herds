...
libs <- c('data.table', 'reactable', 'tidyr')
...
mk_table <- function(tab, title){
  ...
  ft.tmp <- reactable(tmp.order[,-'time2recovery.upper50'], pagination = F)
  return(ft.tmp)
}
```
...
This repository contains the code for analyses [Biodiversity Pathways](https://biodiversitypathways.ca) did on behalf of Jasper National Park (JNP) to identify potential source herds of Southern Mountain Caribou to start their captive breeding program. The final report can be found [here]().
...
reactable(simp.meta.order[,-"Predicted cow growth"], pagination = F)
...
reactable(yrs2recover.order, pagination = F)
```
